    ...
    [Syntax.Link](node) {
      ...
      if (beforeIndex >= 0) {
        ...
        if (isFullWidth(beforeChar) && parentText[linkStart] !== ' ') {
          report(parent, new RuleError(`全角文字とURLの間にはスペースを入れる必要があります`, {
      ...
      if (afterIndex < parentText.length) {
        ...
        if (isFullWidth(afterChar) && parentText[linkEnd - 1] !== ' ') {
          report(parent, new RuleError(`URLと全角文字の間にはスペースを入れる必要があります`, {
        ...
        if (beforeIndex >= 0) {
          ...
          if (isFullWidth(beforeChar)) {
            ...
            if (hasSpaces && !hasSpaceBefore) {
              ...
              report(node, new RuleError(`全角文字とスペースを含む半角文字列の間にはスペースを入れる必要があります: "${beforeChar}${seq.text.substring(0, 10)}..."`, {
                padding: locator.at(beforeIndex + 1)
              }));
            } else if (!hasSpaces && hasSpaceBefore) {
              // Should not have space but does
              report(node, new RuleError(`全角文字とスペースを含まない半角文字列の間にはスペースを入れないでください: "${beforeChar} ${seq.text}"`, {
                padding: locator.at(beforeIndex + 1)
              }));
            }
          }
        ...
        if (afterIndex < text.length) {
          ...
          if (isFullWidth(afterChar)) {
            ...
            if (hasSpaces && !hasSpaceAfter) {
              // Should have space but doesn't
              report(node, new RuleError(`全角文字とスペースを含む半角文字列の間にはスペースを入れる必要があります: "...${seq.text.substring(seq.text.length - 10)}${afterChar}"`, {
                padding: locator.at(afterIndex)
              }));
            } else if (!hasSpaces && hasSpaceAfter) {
              // Should not have space but does
              report(node, new RuleError(`全角文字とスペースを含まない半角文字列の間にはスペースを入れないでください: "${seq.text} ${afterChar}"`, {
